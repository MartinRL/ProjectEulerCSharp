    ...
    public class Problem014
    {
        ...
        public void should_find_the_starting_number_less_than_1000000_that_produces_the_longest_chain()
        {
            var result = 2.ToUint(999999)
                .Select(t => new {Term = t, Count = GetChainCountForR(t)})
        ...
        private static ulong GetChainCountForR(uint startingNumber, ulong count = (ulong)0)
        {
            if (startingNumber == 1)
                return count;

            return GetChainCountForR(startingNumber % 2 == 0 ? startingNumber / 2 : 3 * startingNumber + 1, ++count);
        }
